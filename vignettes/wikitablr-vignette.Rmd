...
However, users do have the option to read and clean their table separately. In this vignette, I detail all of my cleaning functions, and show how they are used in `read_wiki_table()`.
...
If you want to clean the names of all the tables

```{r}
episode_list <- bobs_burgers %>%
  filter(stringr::str_detect(class, "wikiepisodetable"))

all_episodes <- episode_list %>%
  pull(table) %>%
  bind_rows()
```
